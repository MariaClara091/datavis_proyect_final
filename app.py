...
if page == "Inicio / Contexto":
    st.title("Visualización Proyecto UVB - Autores: María Clara Ávila y Mateo José Giraldo")
...
if page == "Carga & ETL":
    st.title("ETL (Extract, Transform, Load)")
    st.markdown("""
...
if page == "Pruebas estadísticas":
    ...
    st.subheader("Normalidad (Kolmogorov–Smirnov) para variables numéricas")

    ks_resultados = {}
    for col in datos_limpios.select_dtypes(include=[np.number]).columns:
        x = datos_limpios[col].dropna()
        if x.nunique() > 1:
        # Escalamos la variable antes de la prueba (media=0, desviación=1)
            x_scaled = (x - x.mean()) / x.std()
            ks_stat, p_val = stats.kstest(x_scaled, 'norm')
            ks_resultados[col] = (round(p_val,4), "Normal" if p_val>0.05 else "No normal")

    # Convertimos resultados a DataFrame
    ks_tabla = pd.DataFrame(ks_resultados).T
    ks_tabla.columns = ["KS_pvalue", "Normalidad"]

    # Mostramos la tabla en la app
    st.dataframe(ks_tabla)
...
if page == "Conclusiones":
    st.title("Interpretación y conclusiones del análisis UVB")

    st.markdown("""
    ### Mejora en la calidad de los datos
    Los histogramas muestran la mejora en la calidad de los datos después del proceso de imputación.
    Ahora las distribuciones presentan valores físicamente posibles y patrones coherentes con el
    comportamiento esperado de variables meteorológicas (por ejemplo, asimetría natural en precipitación
    o radiación).

    ---

    ### Estacionalidad de la radiación UVB
    - El gráfico muestra un **máximo de radiación UVB alrededor del mes de enero**, correspondiente
      a la primera temporada seca del año.
    - Se observa una **disminución hacia marzo y abril**, coincidiendo con la primera temporada de lluvias.
    - Luego inicia un aumento progresivo entre **junio y agosto**, segunda temporada seca del año.

    Estos hallazgos apoyan la **hipótesis H1**, que plantea que la radiación UVB presenta picos durante los
    meses secos debido a la influencia de la nubosidad y la precipitación en la atenuación de la radiación
    ultravioleta.

    ---

    ### Hipótesis H2 — Precipitación vs UVB
    - Se confirma una **correlación negativa moderada** entre la precipitación y la radiación UVB.  
    - Esto significa que **a mayor precipitación, menor radiación UVB**, debido a la mayor cobertura de nubes
      que bloquean parte de la radiación solar.

    ---

    ### Hipótesis H3 — Presión superficial vs UVB
    - Se encontró una **correlación positiva débil** (r = 0.079) entre presión y radiación UVB,
      indicando que solo el 0.6% de la variabilidad en la radiación UVB puede explicarse por cambios
      en la presión superficial.
    - Aunque la relación es directa, es extremadamente débil.  
      Esto sugiere que la presión por sí sola **no es un predictor relevante** del comportamiento de la radiación UVB,
      aunque valores altos de presión pueden estar asociados con cielos despejados.

    ---

    ### Otras correlaciones encontradas
    - **UVB vs Hora del día:** correlación positiva moderada → mayor radiación en horas centrales del día.  
    - **Día del año vs Precipitación:** correlación negativa fuerte → patrones estacionales de lluvia muy marcados.  
    - **UVB vs Día del año:** correlación positiva → variación estacional de la radiación.  
    - **UVB vs Radiación de onda larga:** correlación positiva moderada y significativa → ambas responden a la
      transparencia atmosférica y la nubosidad.

    ---

    ### Resultados de ANOVA y pruebas de homogeneidad
    - **ANOVA:** no se encontraron diferencias estadísticamente significativas en los niveles de radiación UVB
      entre los diferentes meses.
    - **Prueba de Levene:** se encontró **heterogeneidad de varianzas** entre meses, lo cual puede afectar los
      resultados de ANOVA y explicar que, aunque hay patrones estacionales, las diferencias no sean
      estadísticamente significativas.

    ---

    ### Estadísticas descriptivas mensuales
    - La **mediana de UVB** es igual a 0 en casi todos los meses → más del 50% de los valores son cero (noches o días nublados).  
    - La **media de la radiación UVB** aumenta de enero (0.396) a abril (0.413), lo cual puede deberse a mayor insolación.  
    - La **presión superficial** muestra poca variación entre meses → clima relativamente estable.  
    - La **precipitación** aumenta de enero (0.365 mm) a abril (4.605 mm) → inicio de la temporada lluviosa.

    ---

    ### Conclusión general
    La radiación UVB es generalmente baja, aunque existen periodos y horas de exposición intensa.
    La presión atmosférica se mantiene estable y las precipitaciones aumentan progresivamente
    desde invierno hasta primavera, evidenciando el patrón climático local.

    En conjunto, los resultados **confirman parcialmente las hipótesis planteadas**, mostrando
    la clara influencia de la nubosidad y la precipitación sobre la radiación UVB, y descartando
    la presión superficial como un factor explicativo relevante.
    """)
